  ...
  describe SnapshotAccess do
    ...
    describe "#find_by_aggregate_id" do
      it "returns the correct data" do
        event_access.store_events(events[0])
        events = event_access.find_events_by_aggregate_id(aggregate_id)
        access.record_snapshot(aggregate_id, { aggregate_version: 1, event_data: "data" })
    ...
    describe "#record" do
      context "when the aggregate doesn't exist" do
        it "raises an error" do
          expect { access.record_snapshot("qux", "data") }.to raise_error(Errors::SnapshotDataError)
        end
      end
      context "when the aggregate exists" do
        ...
        context "when the snapshot isn't fresh" do
          context "when the versions match" do
            it "records a new snapshot" do
              ...
              expect(access.find_by_aggregate_id(aggregate_id)).to be_nil
              access.record_snapshot(aggregate_id, { aggregate_version: 1, event_data: "data"})
              event_access.store_events(events[1])
        ...
        context "when the snapshot is fresh" do
          it "doesn't record a snapshot" do
            ...
            access.record_snapshot(aggregate_id, { aggregate_version: 2, event_data: "data"})
            access.record_snapshot(aggregate_id, { aggregate_version: 2, event_data: "new_data"})
            snapshot = access.find_by_aggregate_id(aggregate_id)
            expect(snapshot.snapshot_data).to eq("data")
          end
        end
    ...
    it "can write and read snapshots" do
      snapshot_id = access.record_snapshot(aggregate_id, { aggregate_version: 0, event_data: "data" })
